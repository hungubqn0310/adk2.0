...
# class ProductSearchAgent(LlmAgent):
    ...
    async def _run_async_impl(
        ...
        async for event in self.product_search.run_async(ctx):
            ...
            print(f"[DEBUG] is func response: {all([p.function_response is not None for p in event.content.parts])}")
            ...
            if not event.partial and any(p.function_call for p in event.content.parts):
                search_queries = [
        ...
        if search_queries:
            tasks = [
                ...
                asyncio.create_task(UnifiedProductSearchTool().run_async(args=query.args, tool_context=ctx))
